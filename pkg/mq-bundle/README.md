...
/** @var Formapro\MessageQueue\Client\MessageProducer $messageProducer **/
...
As you read before you must keep running `formapro:message-queue:consume` command and to do this best
...
`formapro:message-queue:consume` command and cares about relaunch if instance has dead by any reason.

```ini
[program:pf_message_consumer]
command=/path/to/app/console --env=prod --no-debug formapro:message-queue:consume
...
![Component structure](./Resources/doc/component_structure_diagram.png "The MessageQueue component structure")
